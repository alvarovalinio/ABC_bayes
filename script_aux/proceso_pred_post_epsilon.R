...
# utilizando todos los titas que da el meta - modelo, filtrar por epsilon
...
# Credibilidad "lindo"
datos %>% ggplot(aes(x=age))+geom_line(aes(y=fx),color="red")+geom_ribbon(aes(ymin=int_inf,ymax=int_sup),alpha=0.2)


#obs <- read.csv(here("Materials/datos","asfr_hutterites.csv"))
...
datos %>% ggplot(aes(x=age))+geom_line(aes(y=fx),color="red")+
  geom_ribbon(aes(ymin=int_inf,ymax=int_sup),alpha=0.2)+geom_line(aes(y=obs),linetype="dashed")
