...
```{r}
obs <- readr::read_csv(here::here("trabajo_final/datos","asfr_hutterites.csv"))
```
...
res_dir <- file.path("resultados", paste0("ini_c_", ini_c), paste0("ne_", ne), paste0("N_", N))
post_candidates <- readr::read_rds(here::here("trabajo_final/resultados/ini_c_2000/ne_10/N_520/evals", "evalued_points.rds"))
...
for (i in 1:nrow(accepted)){
  ruta <- paste0("resultados/ini_c_2000/ne_10/N_520/param_set_",accepted$id[i]+100,"/fx.Rdata") 
...
# Loop para cargar datos de carpetas
for (i in 1:520){
      ruta <- paste0("resultados/ini_c_2000/ne_10/N_520/param_set_",i+100,"/fx.Rdata") 
      y_pred <- read_rds(ruta) %>% data.table()
      colnames(y_pred) <- c("age",paste0("fx_",i+100))
      setkey(y_pred,age)
      setkey(edad,age)
